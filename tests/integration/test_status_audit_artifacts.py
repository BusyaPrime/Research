...
def test_status_audit_files_exist(repo_root: Path) -> None:
    ...
    assert (repo_root / "docs" / "status" / "spec_coverage_map.yaml").exists()
...
def test_machine_readable_status_lists_operational_gaps_and_simplifications(repo_root: Path) -> None:
    ...
    assert payload["ten_out_of_ten_status"]["fully_closed"] is False
    assert payload["ten_out_of_ten_status"]["completion_percent_estimate"] >= 90
    assert payload["ten_out_of_ten_open_gaps"]
...
def test_spec_gap_audit_explicitly_states_not_fully_done(repo_root: Path) -> None:
    ...
    assert "по расширенному критерию" in content
