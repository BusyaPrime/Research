...
from alpha_research.dataset.assembly import build_dataset_manifest, build_gold_panel
...
from alpha_research.pipeline.runtime_reporting import persist_reporting_artifacts
...
def execute_operational_command(
    ...
    *,
    ...
    dataset_manifest = build_dataset_manifest(
        ...
        dataset_version=experiment.dataset_version,
        primary_label=experiment.label,
        parquet_path=gold.parquet_path,
        feature_columns=feature_columns,
    )
    artifacts.append(
        _persist_json_artifact(paths.root, manifests_dir / "dataset_manifest.json", "dataset_manifest", dataset_manifest.__dict__)
    )
    artifacts.append(
        StageArtifact(
            name="gold_panel",
    ...
    reporting_result = persist_reporting_artifacts(
        root=paths.root,
        manifests_dir=manifests_dir,
        report_dir=report_dir,
        report_sections_dir=report_sections_dir,
        reporting_config=loaded.bundle.reporting,
    ...
    report_path = reporting_result.report_path
    report_html_path = reporting_result.report_html_path
    report_bundle_path = reporting_result.report_bundle_path
    section_index_path = reporting_result.section_index_path
    section_artifacts = reporting_result.section_artifacts
    pending_formats = reporting_result.pending_formats
    artifacts.extend(reporting_result.stage_artifacts)
    ...
    review_bundle = ReviewBundle(
        ...
        key_metrics={
            ...
            "primary_model_tier": str(primary_model_metadata.tier),
            "net_sharpe": (
                None
                if portfolio_metrics.set_index("metric")["value"].to_dict().get("net_sharpe") is None
                else float(portfolio_metrics.set_index("metric")["value"].to_dict().get("net_sharpe"))
            ),
            "max_drawdown": (
                None
                if portfolio_metrics.set_index("metric")["value"].to_dict().get("max_drawdown") is None
                else float(portfolio_metrics.set_index("metric")["value"].to_dict().get("max_drawdown"))
            ),
            "ablation_rows": int(len(ablation.results)),
            "approval_status": str(approval_summary["status"]),
        },
