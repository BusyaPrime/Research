...
- стабилизировать полный end-to-end pipeline run на совсем чистом окружении уже через live adapter path;
- добить release-guards вокруг внешних adapters, secrets flow и vendor-side operational path;
- отдельно прогнать operational soak против реального поставщика, а не только против local configured fixtures.
