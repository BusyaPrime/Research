...
### Runtime policy теперь строгий по конструкции, а не “по договоренности”

Еще один важный хвост был в том, что runtime местами все еще вел себя слишком снисходительно. Такое удобно ровно до первого серьезного аудита: дальше начинается история про completed-run, который на самом деле quietly degraded somewhere in the middle.

Что поменяли:

- operational experiment теперь выбирается явно через `runtime.operational_experiment_key`;
- unsupported model в operational path больше не приводит к подбору чего-то “хотя бы рабочего”;
- split engine получил явный `ValidationProtocol` и больше не пропускает маленький первый fold через `pass`;
- OOF слой теперь пишет data-usage trace и отдельный evaluation manifest;
- synthetic runtime отделен capability-классом `fixture_only` и не проходит release verifier как release-grade output.

Почему это важно:

Пока система умеет выглядеть более законченной, чем она есть, она опасна не только для пользователя, но и для автора. Строгий runtime policy нужен не для драматизма, а чтобы красивую ложь ломать до того, как она попала в отчет.

### Smoke-профиль пришлось ужать по ablation, но сделали это честно

После ужесточения operational path стало видно, что regression smoke тратит непропорционально много времени на ablation suite. Сам по себе full ablation нужен и остается в боевом контуре, но для smoke это уже начинало напоминать интеграционный тест, который решил пожить полноценной жизнью отдельно от CI.

Что сделали:

- вынесли лимиты `ablation_max_feature_family_scenarios` и `ablation_max_preprocessing_scenarios` в `runtime.release_smoke`;
- full runtime при этом не урезали;
- в ablation добавили кэш по одинаковым `(features, preprocessing)` сценариям, чтобы не гонять один и тот же baseline повторно только ради другой подписи в таблице.

Почему так:

Smoke должен проверять stitching между слоями, а не соревноваться с полноценным research run в терпении. Тут важно было ускорить именно smoke-профиль, а не притвориться, что full ablation внезапно никому не нужен.

### Statistical skepticism перестал быть “когда-нибудь потом”

До этого evaluation слой был уже полезным, но все еще слишком охотно радовался красивым метрикам. Для alpha research это опасная привычка: такие системы редко умирают от синтаксической ошибки, зато постоянно пытаются влюбиться в удачный шум.

Что добавили:

- bootstrap confidence intervals для predictive и portfolio метрик;
- `probabilistic_sharpe_ratio` и `deflated_sharpe_ratio`;
- hypothesis registry по моделям и FDR-контроль через Benjamini-Hochberg;
- prediction correlation matrix между моделями;
- stability gates по regime, cost robustness, ablation resilience, concentration и capacity clipping;
- machine-readable `approval_summary`, который уже пытается ответить не “красиво ли выглядит отчет”, а “что делать с этой гипотезой дальше”.

Почему это важно:

Research platform взрослеет не в тот момент, когда умеет строить еще один график. Она взрослеет, когда начинает системно подозревать собственные хорошие результаты. Этот слой как раз про это.

### Metamorphic tests нужны, потому что leakage любит жить в инвариантах

Обычных integration-тестов здесь уже много, но этого недостаточно. Для временных и OOF-контуров особенно важно проверять не только happy path, но и вещи вида “поменяли порядок строк”, “добавили мусорную колонку”, “прогнали тот же seed второй раз”.

Что добавили:

- split generation не зависит от порядка строк во входной панели;
- OOF-предсказания воспроизводимы при одном и том же seed;
- нерелевантные колонки не должны менять прогноз, если feature list зафиксирован.

Почему так:

Если такие инварианты не проверять отдельно, они обычно ломаются тихо. А тихая поломка в quant-пайплайне — это почти всегда не баг в интерфейсе, а баг в доверии к результату.

### Model layer теперь явно tiered, а не просто “еще несколько моделей рядом”

Когда model zoo начинает расти без реестра и без метаданных, он быстро превращается в стопку классов, которые вроде бы существуют, но плохо отвечают на вопрос “какая у них роль в research loop и чем они вообще отличаются по контракту”.

Что сделали:

- завели `src/alpha_research/models/registry.py` с machine-readable metadata по моделям;
- развели слой на sanity baseline и более серьезный tier с advanced linear/ranking path;
- добавили `elastic_net_regression` и `rank_ridge_regression` как operationally supported модели, а не как заметку на будущее;
- начали сохранять model registry и model stability diagnostics в runtime artifacts.

Почему так:

Если model layer не описан явно, дальше начинается классическая история “эта модель вроде работала полгода назад, но зачем она тут до сих пор и как ее правильно сравнивать с соседями — уже никто не помнит”.

### Spec coverage лучше держать как артефакт, а не как обещание

После того как проект дошел до состояния “по `MASTER_SPEC` уже закрыт”, появилась другая проблема: слишком легко сказать “все уже enforced”, не показывая быстро, где именно это живет в коде и какими тестами держится.

Что сделали:

- добавили `docs/status/spec_coverage_map.yaml`;
- связали ключевые clause'ы с `implemented_in`, `tested_by` и `surfaced_in`;
- завели отдельную проверку, что эти ссылки не протухают.

Почему так:

Большая спецификация хороша ровно до момента, пока между ней и кодом не образуется уютная пропасть. Coverage map нужен именно для того, чтобы эта пропасть не успела стать традицией.
