...
### Figure-артефакты в reporting слое пока не дорендерены

Статус: `TEMPORARY SIMPLIFICATION`

Сейчас runtime собирает нормальный report bundle: markdown-отчет, HTML-версию, persisted sections, section index и figure index. Но сами figures пока не рендерятся как отдельные файлы, а индексируются как ожидаемые артефакты со статусом `stub_not_generated`.

Почему так:

Сначала нужно было замкнуть честный отчетный контракт и provenance по артефактам. Иначе получался старый добрый вариант: отчёт вроде есть, но понять из него, где какая секция живет и каких выходов не хватает, можно только по запаху.

Что это значит для результата:

- секции отчета и форматы теперь сохраняются явно;
- review bundle стал полезнее для ревью и release-check;
- визуальный слой figures еще не доведен до полноценного рендера.

Что нужно сделать дальше:

- добавить renderer для mandatory figures;
- привязать figure artifacts к diagnostics parquet/json слоям;
- включить figure generation в operational path и release gates.
